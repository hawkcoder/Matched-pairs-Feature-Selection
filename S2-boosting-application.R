...
Response = read.table("./_data/Relapse_time2.txt", sep = ";", header=T)
time = Response[,2]
...
new=read.table("./_data/mydata.txt",header=T)
time.long = NULL
for (i in 1:35) {
...
# Subsetting the data 
# *************************
#Set this to zero to select early relapse cases, one for late relapse cases, otherwise entire data is desired
early_late = 2 
if (early_late==0) {
  newdata = newdata[which(newdata[,22285] == 1),] #Early relapse subset
} else if (early_late==1) {
  newdata = newdata[which(newdata[,22285] == 2),] #Late relapse subset
} else {}
...
id1=unique(newdata[,1])
a = sample(id1, length(id1)*0.8)
b = id1[-a]
...
niter.cv = dim(res$mat.test.pred)[2]
t.error = NULL
c.error = NULL

for (i in 1:niter.cv){
  # i = 1
  cvpred = res$mat.test.pred[, i]
  cvtest = res$vtest
...
niter.cv = min(dim(CV.boost$cv1$mat.test.pred)[2], dim(CV.boost$cv2$mat.test.pred)[2],
               dim(CV.boost$cv3$mat.test.pred)[2], dim(CV.boost$cv4$mat.test.pred)[2],
               dim(CV.boost$cv5$mat.test.pred)[2], dim(CV.boost$cv6$mat.test.pred)[2],
               dim(CV.boost$cv7$mat.test.pred)[2], dim(CV.boost$cv8$mat.test.pred)[2],
               dim(CV.boost$cv9$mat.test.pred)[2], dim(CV.boost$cv10$mat.test.pred)[2])

t.error = NULL
c.error = NULL
for (i in 1:niter.cv){
  cvpred = c(CV.boost$cv1$mat.test.pred[,i], CV.boost$cv2$mat.test.pred[,i],
             CV.boost$cv3$mat.test.pred[,i], CV.boost$cv4$mat.test.pred[,i],
             CV.boost$cv5$mat.test.pred[,i], CV.boost$cv6$mat.test.pred[,i],
             CV.boost$cv7$mat.test.pred[,i], CV.boost$cv8$mat.test.pred[,i],
             CV.boost$cv9$mat.test.pred[,i], CV.boost$cv10$mat.test.pred[,i])
  
  cvtest = c(CV.boost$cv1$vtest, CV.boost$cv2$vtest,
             CV.boost$cv3$vtest, CV.boost$cv4$vtest,
             CV.boost$cv5$vtest, CV.boost$cv6$vtest,
             CV.boost$cv7$vtest, CV.boost$cv8$vtest,
             CV.boost$cv9$vtest, CV.boost$cv10$vtest)
